...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channels=3, output_size=2, alpha=1.0, beta=40, base=1.0):
...
def ResNet_size(size=10, in_channels=3, output_size=2, alpha=1.0, beta=40, base=1.0):
    if size == 10:
...
def fit(epochs, max_lr, model, train_loader, val_loader, weight_decay=0.0, grad_clip=None, opt_func=torch.optim.SGD):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.70, patience=2, verbose=True)
...
reference_embeddings, reference_labels = Network_model.extract_embedding(train_dataset)
...
query_embeddings, query_labels = Network_model.extract_embedding(test_dataset)
''' Uncomment to plot the embeddings
if output_dim == 2:
...
acc = Metrics(query_labels, y_pred)
